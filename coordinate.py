...
ax.set_title("Massive Geodesics", color=TEXT_COLOR, fontsize=16, pad=14)
...
def sync_sliders_with_selected():
    ...
    v_slider.set_val(trajectories[selected_index]["v"])
    syncing_sliders = False
...
def redraw(_value=None):
    ...
    for index, line in enumerate(lines):
        if index < len(trajectories):
            ...
            current_v = trajectories[index]["v"]
            traj = massive_geodesics(current_b, current_v, current_m)
            ...
            line.set_label(f"#{index + 1}: b={current_b:.1f}, v={current_v:.2f}")
        else:
            line.set_data([], [])
...
def add_trajectory(_event):
    ...
    trajectories.append({"b": b_slider.val, "v": v_slider.val})
...
b_axis = plt.axes((0.04, 0.24, 0.13, 0.012), facecolor=SLIDER_TRACK)
v_axis = plt.axes((0.04, 0.18, 0.13, 0.012), facecolor=SLIDER_TRACK)
m_axis = plt.axes((0.04, 0.12, 0.13, 0.012), facecolor=SLIDER_TRACK)
add_axis = plt.axes((0.04, 0.33, 0.13, 0.045), facecolor=PANEL_BG)
...
for slider in (b_slider, v_slider, m_slider):
